...
int main() {
    ...
    while (t-- > 0) {
        ...
        str reversed_a(a.rbegin(), a.rend());
        str reversed_b(b.rbegin(), b.rend());
        size_t pos = reversed_a.find(reversed_b);
        ...
        lli last_index = a.size() - pos - b.size();

        lli len_b = b.size();
        lli cost = last_index; // Cost for characters before the last occurrence
        lli remaining = a.size() - (last_index + len_b);
        cost += remaining * (len_b + 1);
