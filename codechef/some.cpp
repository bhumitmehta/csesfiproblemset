...
int main(){
    ...
    while(t-->0){
        ...
        int x= input();
        int k=input();
        std::vector<char> vec = take_vector<char>(n);
        vecint subsequneceCount(n,0);
        ...
        for(int i=0;i<n;i++){
            if(vec[i]=='1'){
                ...
                subsequneceCount[i] += oneCount;
            }
        }
        int subractor = 0;
        for(int i=n-1;i>=0;i--){
           if(subsequneceCount[i]>0){
               int diif = subsequneceCount[i] - subsequneceCount[i]%k;
               
           }
        }
        print_vector(subsequneceCount);
