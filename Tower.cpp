...
template <typename T=lli>
T nearestBinarySearch(std::vector<T> arr, T target){
    T left = 0;
    T right = arr.size()-1;
    while(left<=right){
        ...
        if(arr[mid] == target){
            while(mid<arr.size()-1 && arr[mid+1] == target){
                mid++;
            }
            return mid+1;
        }
...
int main(){
    int n = input();
    veclli arr = take_vector<lli>(n);
    veclli maxStack = {arr[0]};
    for(int i=1;i<n;i++){
        if(arr[i]>maxStack.back()){
            // print("here");
            
            maxStack.push_back(arr[i]);
            // print_vector(maxStack);
        }else{
            // print("here1");
            
            lli val = nearestBinarySearch(maxStack,arr[i]);
            // print(val,"\n");
            if (val == -1) {
                maxStack.push_back(arr[i]);
            }else if(val > maxStack.size()-1){
                maxStack.push_back(arr[i]);
            }
            else {
            maxStack[val]   = arr[i];
