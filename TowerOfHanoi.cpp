...
// Function to solve the Tower of Hanoi problem and print the moves
void TowerOf(int n, int source, int destination, int free) {
    if (n == 0) {
        return;
    }
    TowerOf(n - 1, source, free, destination);
    print(source," ");print(destination);
    TowerOf(n - 1, free, destination, source);
}

int main() {
    int n = input(); // Read the number of disks
