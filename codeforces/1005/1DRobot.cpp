...
int findClosestX(int z, int y) {
    if (z == 0) {
        throw std::invalid_argument("z cannot be zero.");
    }

    int x = std::max(1, static_cast<int>(std::round(-static_cast<double>(y) / z)));
    ...
    // Check both x and x+1 for better accuracy
    int x1 = x;
    int x2 = x + 1;

    int val1 = std::abs(z * x1 + y);
    int val2 = std::abs(z * x2 + y);

    return (val1 <= val2) ? x1 : x2;
}

int main(){
    int t = input();
    while(t-->0){
        int n =input(),x = input(),k=input();
        str s = input<str>();
        int count = 0;
        int net =0;
        for(const auto& p:s ){
