...
void calculatebose(std::vector<std::pair<int, std::vector<int>>>& bosses, int i) {
    if (bosses[i].second.empty()) {
        return;
    }

    for (int j = 0; j < bosses[i].second.size(); j++) {
        int subordinate = bosses[i].second[j];
        calculatebose(bosses, subordinate);
        bosses[i].first += bosses[subordinate].first + 1; // +1 to count the subordinate itself
    }
    return;
}
...
int main() {
    ...
    vector<pair<int, vector<int>>> bosses(n, {0, vector<int>()});

    for (int i = 0; i < n - 1; i++) {
        bosses[b[i] - 1].second.push_back(i + 1);
    }

    calculatebose(bosses, 0);
